...
export function Dashboard({ servers, busy, act, onError }: { servers: Server[]; busy: boolean; act: (server: Server, action: string) => Promise<void>; onError: (error: unknown) => void }) {
  ...
  return <div className="page-stack">
    <div className="page-heading"><div><p className="eyebrow">CONTROL PLANE</p><h1>{tr('Selamat datang kembali', 'Welcome back')}</h1><p>{tr('Pantau server dan aktivitas node dari satu tempat.', 'Monitor your servers and node activity from one place.')}</p></div><ActionButton icon={CirclePlus} onClick={() => navigate('/servers?create=1')}>{tr('Buat Server', 'Create Server')}</ActionButton></div>
    <div className="dashboard-grid">
      <section className="surface server-list-panel"><div className="section-heading"><div><h2>{tr('Server Minecraft', 'Minecraft Servers')}</h2><p>{tr(`${servers.filter((server) => server.state === 'running').length} dari ${servers.length} berjalan`, `${servers.filter((server) => server.state === 'running').length} of ${servers.length} running`)}</p></div><button className="text-button" onClick={() => navigate('/servers')}>{tr('Lihat semua', 'View all')}<ArrowRight /></button></div>
        {servers.length === 0 ? <EmptyState icon={ServerIcon} title={tr('Belum ada server', 'No servers yet')} description={tr('Buat server Minecraft pertama Anda untuk memulai.', 'Create your first Minecraft server to get started.')} action={{ label: tr('Buat Server', 'Create Server'), onClick: () => navigate('/servers?create=1') }} /> : <div className="server-card-list">{servers.map((server) => <ServerCard key={server.id} server={server} metrics={metrics[server.id]} busy={busy} act={act} />)}</div>}
      </section>
      <section className="surface node-overview"><div className="section-heading"><div><h2>{tr('Ringkasan Node', 'Node Overview')}</h2><p>local</p></div><span className="unavailable">{tr('Telemetri tidak tersedia', 'Telemetry unavailable')}</span></div>
...
export function ServerCard({ server, metrics, busy, act }: { server: Server; metrics: Metrics | null | undefined; busy: boolean; act: (server: Server, action: string) => Promise<void> }) {
  ...
  return <article className="server-card" onClick={() => navigate(`/servers/${server.id}/console`)}>
    ...
    <div className="server-quick-actions" onClick={(event) => event.stopPropagation()}>
      ...
      <ActionButton size="sm" variant="ghost" icon={Terminal} aria-label={tr(`Buka console ${server.name}`, `Open ${server.name} console`)} title={tr('Buka console', 'Open console')} onClick={() => navigate(`/servers/${server.id}/console`)} />
    </div>
  </article>;
}
