...
import type { Capacity, Notification, Server, Session } from '../types';
import { StatusBadge, normalizeStatus } from '../components/ui/StatusBadge';
import { api } from '../api';
import { formatDate } from '../format';

export function AppShell({ children, servers, capacity, session, onLogout }: { children: ReactNode; servers: Server[]; capacity: Capacity | null; session: Session; onLogout: () => Promise<void> }) {
  ...
  const loadNotifications = useCallback(() => api<Notification[]>('/api/v1/notifications').then(setNotifications).catch(() => undefined), []);
  ...
  const mobilePrimary = activeServer
    ? [{ to: '/servers', label: tr('Server', 'Servers'), icon: ServerIcon }, ...contextualNav.slice(0, 3)]
    : [globalNav[0], globalNav[1], ...(session.role === 'user' ? [globalNav[2]] : [{ to: '/admin/capacity', label: tr('Kapasitas', 'Capacity'), icon: Monitor }]), { to: '/billing', label: tr('Pesanan', 'Orders'), icon: CreditCard }];
  const mobileMore = [...globalNav, ...contextualNav].filter((item) => !mobilePrimary.some((primary) => primary.to === item.to));
  ...
  return <div className={sidebarCollapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
    <aside className="sidebar" aria-label={tr('Navigasi utama', 'Main navigation')}>
      ...
      {session.role === 'owner' && <section className="node-mini" aria-label={tr('Informasi node', 'Node information')}>
        <div><Monitor /><span><b>local</b><small>{capacity?.host ? `${capacity.host.cpuPercent.toFixed(0)}% CPU host` : tr('Telemetri tidak tersedia', 'Telemetry unavailable')}</small></span></div>
    ...
    <div className="app-column">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <div className="notification-wrap"><button className="icon-button notification" title={tr('Notifikasi operasional', 'Operational notifications')} aria-label={tr('Buka notifikasi', 'Open notifications')} onClick={() => setNotificationsOpen((value) => !value)}><Bell />{unread > 0 && <span>{Math.min(unread, 99)}</span>}</button>{notificationsOpen && <section className="notification-panel"><header><div><b>{tr('Notifikasi', 'Notifications')}</b><small>{tr(`${unread} belum dibaca`, `${unread} unread`)}</small></div>{unread > 0 && <button onClick={async () => { await api('/api/v1/notifications', { method: 'POST', body: JSON.stringify({ action: 'read_all' }) }); await loadNotifications(); }}>{tr('Tandai dibaca', 'Mark all read')}</button>}</header><div>{notifications.length === 0 ? <p>{tr('Tidak ada peringatan operasional.', 'No operational alerts.')}</p> : notifications.map((item) => <button key={item.id} className={`notification-item ${item.severity} ${item.readAt ? '' : 'unread'}`} onClick={async () => { await api(`/api/v1/notifications/${item.id}`, { method: 'POST', body: '{}' }); if (item.serverId) navigate(`/servers/${item.serverId}/console`); setNotificationsOpen(false); await loadNotifications(); }}><span /><div><b>{item.title}</b><p>{item.message}</p><small>{formatDate(item.createdAt)}</small></div></button>)}</div></section>}</div>
