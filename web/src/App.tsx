...
export function App() {
  ...
  return <AppShell servers={servers} session={session} onLogout={async () => { try { await api('/api/v1/auth/logout', { method: 'POST' }); } finally { signOut(); } }}><Routes>
    ...
    <Route path="dashboard" element={<Page title={tr('Dashboard gagal dimuat.', 'Dashboard failed to load.')}><Dashboard servers={servers} busy={busy} act={act} onError={handleError} isAdmin={session.role === 'owner'} /></Page>} />
    <Route path="servers" element={<Page title={tr('Daftar server gagal dimuat.', 'Server list failed to load.')}><ServersPage servers={servers} catalog={catalog} busy={busy} act={act} setBusy={setBusy} canCreate={session.role === 'owner'} onCreated={async () => { showToast(tr('Server dibuat dan provisioning dimulai.', 'Server created and provisioning started.'), 'success'); await loadServers(); }} onError={handleError} /></Page>} />
